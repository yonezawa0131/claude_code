...
| 要素 | 配置 | 役割 |
|---|---|---|
| サイドバーラベル | 左端・縦中央 | セクション全体の見出し（例：「選考フロー」）を縦書き or 横書きで表示（32px相当） |
| STEPバッジ | 各行・左側（小幅ボックス） | ステップ番号を着色背景で強調表示（例：STEP 1）（22px相当） |
| 内容ボックス | 各行・STEPバッジの右（広幅ボックス） | そのステップの内容テキストを枠線付きボックスで表示（24px相当） |
