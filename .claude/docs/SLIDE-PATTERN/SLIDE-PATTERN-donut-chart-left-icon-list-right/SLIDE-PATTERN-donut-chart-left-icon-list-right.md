...
**概要：** コンテンツエリア左側（45%）にドーナツ型グラフ＋テキスト見出し、右側（55%）にアイコン付き縦リスト4項目を配置するデータ可視化レイアウト。グラフで全体像を示しながら、右側の詳細リストで各要素を補足説明する。
...
コンテンツエリアを左右に分割し、左にドーナツグラフ、右にアイコン付きリストを配置する。
    ...
    structure:
      ...
      padding: 32px 80px（高さの約3%）
      gap: 48px
      left-area:
        width: 45%
        align: center, center (flex)
        elements:
          - text-heading (font-size: 28px, bold, #555, text-align: center, 2行)
          - donut-chart-svg (width: 320px, height: 320px, 4分割グレースケール)
      right-area:
        width: 55%
        ...
        elements:
          - icon-list (4項目)
          - each-row:
              - icon-circle (width: 64px, height: 64px, #F0F0F0, border-radius: 50%)
              - text-area:
                  - item-heading (font-size: 26px, bold, #333)
                  - item-description (font-size: 24px, #666)
...
| 要素 | 配置 | 役割 |
|---|---|---|
| テキスト見出し | 左エリア上部 | グラフの内容を2行で説明するキャプション的テキスト（28px相当） |
| ドーナツグラフSVG | 左エリア中央 | 4分割の円グラフで全体の構成比を視覚的に表現 |
| グラフ中央テキスト | ドーナツ内部 | グラフの中心に表示するラベル・概念語（1〜2語、22px相当） |
| アイコン円 | 右エリア各行左端 | 内容に沿ったSVGアイコンを生成して配置するプレースホルダー円 |
| 項目見出し | アイコン右、上段 | 各リスト項目の名称・分類名を太字で表示（26px相当） |
| 項目説明 | アイコン右、下段 | 各項目の補足説明を1行で記述（24px相当） |
