...
- **type**: `#type/skill` `#type/script` `#type/hook` `#type/config` `#type/playbook` `#type/concept` `#type/article`
- **domain**: `#domain/memory` `#domain/vault` `#domain/import` `#domain/infra` `#domain/model`(モデル/effort の選択・運用) `#domain/agent`(エージェント活用の設計・運用)
...
- [[article-karpathy-claude-playbook]] — カーパシー式 Claude 活用術記事(2026-07)の要約と取り入れ候補の抽出
...
- [[success-criteria-loop]] — 成功条件駆動ループ(命令ではなくゴール+検証手段を与えて反復させる)
