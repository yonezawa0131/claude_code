...
- **Python フレームワーク実装（Router / VerificationEngine / MemoryStore 等のコード）**:
  ガイドのコードは実在しないモデル・API を前提とした擬似コードで、そのままでは動かない。
  このリポジトリは設定・知見リポジトリでありフレームワーク開発の場でもない。
  本当にマルチモデル・オーケストレーションが必要になったら、自作せず
  Claude Agent SDK など実在のフレームワークを起点にする。
- **モデル別ルーティング表**: 前提のモデル名・価格が架空。考え方（安いモデルで検証、
  高いモデルは最終成果物のみ）だけ `long-task-state` に反映。
