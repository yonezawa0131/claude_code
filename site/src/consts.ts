...
/**
 * アフィリエイトの識別子。
 * 提携が承認されたらここに入れる。空のままなら、そのショップのボタンは出力されない
 * （タグなしのリンクを出しても成果にならないため、出さないほうがよい）。
 */
export const AFFILIATE = {
  /** AmazonアソシエイトのトラッキングID（例: yourname-22） */
  amazonTag: '',
  /** 楽天アフィリエイトID */
  rakutenId: '',
  /**
   * Amazonのボタンを「カートに入れる」にするか。
   * Amazonの成果条件はクリックから24時間以内のカート投入なので、
   * 検討期間の長い高額商品では有効な可能性がある。
   * ただし読者にとっては唐突な挙動になるため、既定では無効にしている。
   *
   * 注意: この add.html 形式は現行のAmazon公式ヘルプに記載が見当たらない。
   * 昔から使われている形式で動作報告は多いが、正式にサポートされている保証はない。
   * 有効にする場合は、自分のアソシエイトIDで成果が計上されることを確認してから使うこと。
   */
...
/**
 * アクセス解析。
 * 設定するとタグが出力され、プライバシーポリシーの記述も自動で切り替わる。
 */
export const ANALYTICS = {
  /**
   * GA4 の測定ID（G-XXXXXXXXXX）。空ならタグを出力しない。
   * 拡張計測機能が既定で有効なので、外部リンクのクリックは追加実装なしで計測される。
   */
  ga4Id: '',
} as const;
...
export const PR_DISCLOSURE = {
  ...
  /**
   * Amazonアソシエイト運営規約が求める開示文。
   * 景品表示法のステマ規制とは別に、Amazon側の規約として表示義務がある。
   * サイト上の分かりやすい場所（フッター）に出す。
   */
  amazonRequired: `Amazonのアソシエイトとして、${SITE.name}は適格販売により収入を得ています。`,
} as const;
